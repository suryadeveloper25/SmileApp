...
//   navigation: any;
// }
// //  const RAZORPAY_KEY_ID = 'rzp_test_1WPEHHS3q6nLjr';
// // ─── Component ────────────────────────────────────────────────────────────────
...
//     image: 'https://www.vtsmile.in/logo.png', 
//     currency: 'INR',
//     key: 'rzp_live_S5gFX8SSMlhz1p',
//     amount: '50000',
//     name: 'Acme Corp',
//     order_id: '',//Replace this with an order_id created using Orders API.
//     prefill: {
//       email: 'surya55@gmail.com',
//       contact: '7094528454',
//       name: 'surya'
//     },
//     theme: {color: '#53a20e'}
...
const FeesScreen: React.FC<FeesScreenProps> = ({ route, navigation }) => {
  ...
  const [isLoading, setIsLoading] = useState(true);
  const [mobileNo, setMobileNo] = useState('');
  const [feeListHeight, setFeeListHeight] = useState(0);
   ...
   */
  const getStudentData = async (): Promise<void> => {
    try {
      const storedMobile = await AsyncStorage.getItem('mobile');
      setMobileNo(storedMobile ?? '');
      ...
      const response = await axios.post(
        ...
        null,
        {
          params: {
            orgId: orgid,
            studeId: studentId, // ✅ Verify with backend if this is correct param name
  ...
  /**
   * Fetch pending tuition fees
   */
  const pendingTuitionFeeData = async (): Promise<void> => {
    try {
      const response = await axios.post(
        `https://www.vtsmile.in/app/api/students/fee_pending_api`,
        null,
      ...
      // console.log(response.data.fee_pending_dtl, '===response.data.fee_pending_dtl');
      if (response?.data?.isSuccess) {
        setTotalPending(response.data.total_pending?.toString() ?? '0');
        setPendingTuitionFeeList(response.data.fee_pending_dtl ?? []);
      }
    } catch (error) {
      console.log('Error fetching tuition fee:', error);
    }
  };

  /**
   * Fetch pending bus fees
   */
  const pendingBusFeeData = async (): Promise<void> => {
    try {
      const response = await axios.post(
        `https://www.vtsmile.in/app/api/students/bus_fee_pending_api`,
        null,
      ...
      // console.log(response.data.bus_fee_pending_dtl, '=====>response.data.bus_fee_pending_dtl');
      if (response?.data?.isSuccess && response?.data?.bus_fee_pending_dtl) {
        setTotalBusPending(response.data.total_pending?.toString() ?? '0');
        setPendingBusFeeList(response.data.bus_fee_pending_dtl);
      }
    } catch (error) {
  ...
  /**
   * Fetch all bills list
   */
  const getBillsList = async (): Promise<void> => {
    try {
      const response = await axios.post(
        `https://www.vtsmile.in/app/api/students/fee_paid_api`,
        null,
        {
          params: { orgId: orgid, stuId: studentId },
          timeout: 10000,
      ...
      if (response?.data?.isSuccess && response?.data?.data) {
        setBillsList(Array.isArray(response.data.data) ? response.data.data : []);
      }
    } catch (error) {
  ...
  const handlePayment = async (): Promise<void> => {
    const isTuition = activeTab === 'Tuition';
    const amount = isTuition ? totalPending : totalBusPending;

    if (!amount || amount === '0') {
      Alert.alert('Info', 'No pending fees to pay.');
      return;
    }
    ...
    try {
      ...
      const options = {
        ...
        name: studentData.name || 'Student Fee Payment',
        order_id: '',
        prefill: {
          email: 'student@example.com',
          contact: mobileNo || '9999999999',
          name: studentData.name,
        },
        theme: { color: isTuition ? COLORS.primary : COLORS.accent },
      };
      ...
      if (response?.razorpay_payment_id) {
        ...
        // ✅ FIX 2: Refresh fees after successful payment
        await loadData();
      }
    } catch (error: any) {
      if (error?.code !== 'CANCELLED') {
        Alert.alert(
          'Payment Failed',
          error?.description || 'Payment could not be processed'
        );
      }
    }
  };
  ...
  // ✅ FIX 3: Refresh data when screen is focused
  useFocusEffect(
    useCallback(() => {
      loadData();
    }, [loadData])
  ...
  const currentList = isTuition ? pendingTuitionFeeList : pendingBusFeeList;
  const currentTotal = isTuition ? totalPending : totalBusPending;
  const hasPending = currentList && currentList.length > 0;
    ...
    >
      <View style={styles.feeRowLeft}>
        <View
          style={[
            styles.feeDot,
            { backgroundColor: isTuition ? COLORS.primary : COLORS.primary },
          ]}
        />
        <Text style={styles.feeLabel} numberOfLines={2}>
          {isTuition ? item.fee_type : item.fee_type}
        </Text>
      </View>
      <View style={styles.feeRowRight}>
        <Text style={styles.feeAmount}>
          ₹{parseFloat(item.pending_amt || '0').toFixed(2)}
        </Text>
        {!isTuition && item.status && (
          <View
            style={[
        ...
        >
          ...
          <View style={styles.studentCard}>
            ...
            <View style={styles.infoPillsRow}>
              <InfoPill
                icon="group"
                label={`Class ${studentData.std} – ${studentData.section}`}
              />
              <InfoPill icon="list" label={`Roll No. ${studentData.roll}`} />
            </View>
          </View>
          ...
          >
            ...
            {hasPending ? (
              <>
                <View
                  style={styles.feeListContainer}
                  onLayout={(e) => {
                    const height = e.nativeEvent.layout.height;
                    setFeeListHeight(height);
                  }}
                >
                  {currentList && currentList.length > 0 && (
                    currentList.map((item, index) => (
                      renderFeeRow({ item, index })
                    ))
                  )}
                </View>
                ...
                <View style={styles.totalContainer}>
                  ...
                  >
                    ...
                    >
                      ₹ {parseFloat(currentTotal || '0').toFixed(2)}
                    </Text>
                  </View>
                </View>
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.85}
                  // ✅ FIX 4: Proper data passing to BillsList
                  onPress={() =>
              ...
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptySubtitle}>
                  No pending {isTuition ? 'tuition' : 'bus'}{' '}
                  fees.{'\n'}You're up to date.
                </Text>
                <View
                  style={[
                ...
                >
                  <Text style={styles.emptyAmountLabel}>
                    Total Pending Amount
                  </Text>
                  <Text
                    style={[
...
const TabButton = ({
  ...
  >
    ...
    <Text
      style={[
        tabBtnStyles.label,
        active && tabBtnStyles.labelActive,
      ]}
    >
      {label}
    </Text>
